...
from arc.utils import pylogger
...
class MultiViewDUSt3RLitModule(LightningModule):
    ...
    @classmethod
    def load_for_inference(cls, net: torch.nn.Module):
    ...
    def forward(self, views: List[Dict[str, torch.Tensor]]) -> Any:
        return self.net(views, force_no_output_conversion=True)
    ...
    def model_step(
        ...
        # Move data to device
        for view in batch:
            for name in "img pts3d depthmap valid_mask camera_pose camera_intrinsics F_matrix corres".split():
                if name in view:
    ...
    def validation_step(
        ...
        del loss, loss_details, views, preds
