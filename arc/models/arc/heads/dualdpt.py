...
from arc.models.arc.utils.head_utils import (
    Permute,
    create_uv_grid,
    custom_interpolate,
...
class DualDPT(nn.Module):
    ...
    def _reshape_output(self, out_dict: dict, B: int, S: int) -> dict:
        """Reshape output tensors and lists from (B*S, ...) to (B, S, ...)."""
        result = {}
    ...
    def _concat_outputs(self, out_dicts: List[dict]) -> dict:
        """Concatenate outputs from multiple chunks."""
        result = {}
        for k in out_dicts[0].keys():
            if isinstance(out_dicts[0][k], list):
                # For list values, concat corresponding elements
    ...
    def _view_output(self, out_dict: dict, B: int, S: int) -> dict:
        """View output tensors and lists from (B*S, ...) to (B, S, ...)."""
        result = {}
        for k, v in out_dict.items():
            if isinstance(v, list):
    ...
    def _forward_impl(
        ...
        # Auxiliary head (multi-level inside)
        # Process all levels and store in list for multi-level supervision
        # Keep each level at its native resolution (no upsampling)
        aux_pred_list = []
        aux_conf_list = []
        for level_idx, aux_feat in enumerate(fused_aux_pyr):
            # aux_feat = custom_interpolate(
            #     aux_feat, (h_out // 2, w_out // 2), mode="bilinear", align_corners=True
            # )
            if self.pos_embed:
                aux_feat = self._add_pos_embed(aux_feat, W, H)
            # Apply level-specific output conv at native resolution
        ...
        # Use the last level (finest) as the main aux output (backward compatible)
