...
def get_inference_output(args, model, device, filelist, tracks_uv=None, visibility=None, size=None):
    ...
    with torch.no_grad():
        ...
        track_query_idx = getattr(args, "track_query_idx", 11)
        for view in views:
            view["track_query_idx"] = np.array([track_query_idx], dtype=np.int64)
        ...
        for view in views:
            for name in "img pts3d valid_mask camera_pose camera_intrinsics F_matrix corres query_points track_query_idx".split():
                if name not in view:
                    continue
    ...
    for track_npz in tqdm(track_list):
        ...
        try:
            pred_tracks = get_inference_output(
                ...
                tracks_uv=None, visibility=None,
            )
            ...
            pred_tracks = get_inference_output(
                args, model, device, filelist,
                tracks_uv=None, visibility=None, size=336,
            )
        if save_predictions:
        ...
        # Dense: tracks_xyz_world is (T, H, W, 3); use visibility_mask directly
        gt_h, gt_w = tracks_xyz_world.shape[1:3]
        pred_h, pred_w = pred_tracks.shape[1:3]
        if (pred_h, pred_w) != (gt_h, gt_w):
            print(f"Warning: pred {pred_h}x{pred_w} != gt {gt_h}x{gt_w}, resizing with nearest")
            if torch.is_tensor(pred_tracks):
                pred_tracks = F.interpolate(
                    pred_tracks.permute(0, 3, 1, 2),
                    size=(gt_h, gt_w), mode="nearest").permute(0, 2, 3, 1)
            else:
                pred_tracks = (
                    F.interpolate(
                        torch.from_numpy(pred_tracks).permute(0, 3, 1, 2),
                        size=(gt_h, gt_w), mode="nearest")
                    .permute(0, 2, 3, 1).numpy()
                )

        gt_tracks_filtered = tracks_xyz_world[:num_frames, visibility_mask]
        if torch.is_tensor(pred_tracks):
            pred_mask = torch.from_numpy(visibility_mask).to(pred_tracks.device)
            pred_tracks_filtered = pred_tracks[:num_frames][:, pred_mask]
        else:
            pred_tracks_filtered = pred_tracks[:num_frames, visibility_mask]
