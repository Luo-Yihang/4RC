...
from arc.dust3r.utils.geometry import geotrf
...
def get_args_parser():
    ...
    parser.add_argument("--pi3_umeyama_align", action="store_true",
                        help="use Umeyama alignment before evaluation")
    parser.add_argument("--dataset", type=str, default="all",
                        choices=["all", "7scenes", "NRGBD"],
                        help="which dataset(s) to evaluate; 'all' runs both")
    return parser
...
def main(args):
    ...
    checkpoint = args.weights if args.weights else "Luo-Yihang/4RC"
    ...
    with torch.no_grad():
        for name_data, dataset in datasets_all.items():
            ...
            for data_idx in tqdm(idxs):
                ...
                if model_name == "ours" or model_name == "stream3r" or model_name == "arc":
                    ...
                    with torch.no_grad():
                        with torch.autocast('cuda', dtype=torch.bfloat16):
                            # Arc consumes the list of views directly and returns one
                            # dict per view, where "pts" are the world points unprojected
                            # from depth and "conf" is the depth confidence.
                            predictions = model(batch, inference_track=False)

                        all_preds = []
                        for pred in predictions:
                            all_preds.append(
                            {'pts3d': pred['pts'].cpu(), 'conf': pred['conf']}
                            )
                ...
                if args.pi3_umeyama_align:
                    valid_mask = masks_all > 0
                    c, R, t = umeyama(pts_all[valid_mask].T, pts_gt_all[valid_mask].T)
                    pts_all = c * np.einsum('nhwj, ij -> nhwi', pts_all, R) + t.T
